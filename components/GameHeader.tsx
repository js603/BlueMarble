...
export function GameHeader({
    ...
    return (
        <header className="relative z-30 pt-safe-top px-3 sm:px-4 pb-2 bg-slate-900/80 backdrop-blur-md border-b border-white/5 flex flex-col gap-2 shrink-0">
            <div className="flex justify-between items-center h-10 sm:h-12">
                <h1 className="text-lg sm:text-xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">NEBULA</h1>
                        ...
                        <div key={p.id} className={`flex items-center gap-2 p-1.5 pr-4 rounded-full border shadow-sm transition-all min-w-fit shrink-0 ${isActive ? 'bg-slate-800 border-emerald-500 ring-1 ring-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-slate-900/50 border-white/10'} ${p.isBankrupt ? 'grayscale opacity-50' : ''}`}>
                            ...
                            <div className="flex flex-col leading-none">
                                <span className={`text-[10px] font-bold ${isActive ? 'text-white' : 'text-slate-400'}`}>
                                    {p.name} {isMe && '(나)'}
                                    {p.hasEscapeCard && <span className="ml-1 text-[9px] bg-amber-400/20 text-amber-300 px-1.5 py-0.5 rounded-full">🗝️</span>}
                                </span>
